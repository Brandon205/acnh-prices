...
export default function BugAndFish(props) {
    ...
    let mappedAnimals = props.all.map( (animal, id) => <div className="animal" key={id} onClick={() => expand(animal)}>{animal.name} <p className="prices">{formatter.format(animal.price)}</p></div> );
